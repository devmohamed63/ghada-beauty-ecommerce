...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <a 
            href="<?php echo e(route('admin.orders.export', request()->query())); ?>" 
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-pink-100 p-4 md:p-6">
        <form method="GET" action="<?php echo e(route('admin.orders.index')); ?>" class="flex flex-col sm:flex-row gap-4">
            <input 
                type="text" 
                name="search" 
            ...
            >
                ...
                <option value="shipped" <?php echo e(($filters['status'] ?? '') == 'shipped' ? 'selected' : ''); ?>>تم الشحن</option>
                <option value="delivered" <?php echo e(($filters['status'] ?? '') == 'delivered' ? 'selected' : ''); ?>>تم التسليم</option>
    ...
    <div class="flex flex-wrap gap-2 justify-center md:justify-start">
        ...
        ?>
        <?php $__currentLoopData = ['pending', 'confirmed', 'shipped', 'delivered', 'cancelled']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $status): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <a 
                href="<?php echo e(route('admin.orders.index', ['status' => $status] + request()->except('status'))); ?>"
                class="px-3 py-1.5 rounded-lg text-xs font-medium border <?php echo e($statusColors[$status] ?? 'bg-gray-100 text-gray-700 border-gray-300'); ?> hover:shadow-md transition-all <?php echo e(($filters['status'] ?? '') == $status ? 'ring-2 ring-pink-400' : ''); ?>"
            >
                <?php echo e($statusLabels[$status] ?? $status); ?>: <span class="font-bold"><?php echo e($statusCounts[$status] ?? 0); ?></span>
            </a>
    ...
    <div class="md:hidden space-y-4 mb-6">
        ...
        <?php if($ordersCount > 0): ?>
            <?php $__currentLoopData = $ordersList; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $order): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <?php echo $__env->make('admin.components.order-card', ['order' => $order], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        <?php else: ?>
                            ...
                            <tr class="hover:bg-pink-50 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-500"><?php echo e($order->governorate->name_ar ?? ''); ?> - <?php echo e($order->city->name_ar ?? ''); ?></p>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-gray-600"><?php echo e($order->customer_phone); ?></td>
                                <td class="px-6 py-4">
                                    <span class="font-bold text-pink-600"><?php echo e(number_format($order->total, 0)); ?></span>
                                ...
                                <td class="px-6 py-4">
                                    <form method="POST" action="<?php echo e(route('admin.orders.update', $order)); ?>" class="inline-block">
                                        ...
                                        <input type="hidden" name="status_filter" value="<?php echo e($filters['status'] ?? ''); ?>">
                                        <input type="hidden" name="search_filter" value="<?php echo e($filters['search'] ?? ''); ?>">
                                        <select 
                                            name="status" 
...
<?php echo $__env->make('admin.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\freelance\ghada beauty\resources\views/admin/orders/index.blade.php ENDPATH**/ ?>