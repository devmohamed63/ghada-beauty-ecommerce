<?php $__env->startSection('title', 'روتين العناية بالبشرة - اختاري الروتين المناسب لنوع بشرتك | Ghada Beauty'); ?>

<?php $__env->startSection('description', 'اكتشفي الروتين المثالي للعناية بالبشرة حسب نوع بشرتك. روتين صباحي ومسائي مخصص للبشرة الدهنية، الجافة، المختلطة والحساسة. منتجات أصلية 100% من Ghada Beauty.'); ?>
    ...
    <div class="container py-12">
        ...
        <div class="bg-white rounded-2xl shadow-sm p-6 mb-8 border border-pink-50">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                ?>
                <?php if(isset($skinRoutines) && $skinRoutines && is_array($skinRoutines) && count($skinRoutines) > 0): ?>
                <?php $__currentLoopData = $skinRoutines; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $routine): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <button @click="selectSkinType('<?php echo e($routine['type']); ?>')" :class="selectedType === '<?php echo e($routine['type']); ?>' ? 'border-pink-500 shadow-lg scale-105' : 'border-gray-200 hover:border-pink-300'" class="relative overflow-hidden rounded-xl text-center transition-all border-2 group">
                    
                    <div class="relative h-48">
                        <img src="<?php echo e($skinTypeImages[$routine['type']] ?? ''); ?>" alt="روتين العناية بالبشرة <?php echo e($routine['type']); ?> - منتجات Ghada Beauty" width="400" height="300" class="w-full h-full object-cover" loading="lazy">
                        <div :class="selectedType === '<?php echo e($routine['type']); ?>' ? 'bg-gradient-to-t from-pink-600/90 via-purple-500/80 to-pink-400/60' : 'bg-gradient-to-t from-gray-900/70 via-gray-900/50 to-gray-900/30 group-hover:from-pink-600/70 group-hover:via-purple-500/60 group-hover:to-pink-400/40'" class="absolute inset-0 transition-all"></div>
                    </div>
                    
                    
                    <div class="absolute inset-0 flex flex-col items-center justify-center">
                        <div class="text-4xl mb-2 drop-shadow-lg">
                            <?php if($routine['type'] === 'دهنية'): ?> 💧
                            <?php elseif($routine['type'] === 'جافة'): ?> 🌸
                            <?php elseif($routine['type'] === 'مختلطة'): ?> ✨
                            <?php elseif($routine['type'] === 'حساسة'): ?> 🌿
                            <?php endif; ?>
                        </div>
                        <span class="text-white drop-shadow-lg">بشرة <?php echo e($routine['type']); ?></span>
                    </div>
                </button>
...
<script>
window.productsData = <?php echo json_encode($products, 15, 512) ?>;
</script>
<?php $__env->stopPush(); ?>
...
function skinRoutine() {
    return {
        selectedType: '<?php echo e($skinRoutines[0]['type'] ?? 'دهنية'); ?>',
        activeTime: 'morning',
        
        routines: <?php echo json_encode($skinRoutines, 15, 512) ?>,
...
<?php echo $__env->make('front.layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\freelance\ghada beauty\resources\views/front/routine/index.blade.php ENDPATH**/ ?>