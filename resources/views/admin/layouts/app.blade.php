...
<head>
    ...
    <title>@yield('title', 'لوحة التحكم') - Ghada Beauty</title>
        ...
        <nav class="bg-white shadow-sm sticky top-0 z-50 border-b border-pink-100">
            <div class="container mx-auto px-4">
                <div class="flex items-center justify-between h-16">
                    ...
                    <div class="hidden md:flex items-center gap-6">
                        <a href="{{ route('admin.dashboard') }}" class="px-4 py-2 rounded-full transition-colors {{ request()->routeIs('admin.dashboard') ? 'bg-pink-100 text-pink-600 font-medium' : 'text-gray-700 hover:text-pink-500' }}">
                            لوحة التحكم
                        </a>
                        <a href="{{ route('admin.products.index') }}" class="px-4 py-2 rounded-full transition-colors {{ request()->routeIs('admin.products.*') ? 'bg-pink-100 text-pink-600 font-medium' : 'text-gray-700 hover:text-pink-500' }}">
                            المنتجات
                        </a>
                        <a href="{{ route('admin.orders.index') }}" class="px-4 py-2 rounded-full transition-colors {{ request()->routeIs('admin.orders.*') ? 'bg-pink-100 text-pink-600 font-medium' : 'text-gray-700 hover:text-pink-500' }}">
                            الطلبات
                        </a>
                        <a href="{{ route('admin.reports.index') }}" class="px-4 py-2 rounded-full transition-colors {{ request()->routeIs('admin.reports.*') ? 'bg-pink-100 text-pink-600 font-medium' : 'text-gray-700 hover:text-pink-500' }}">
                            الحسابات
                        </a>
                        <a href="{{ route('home') }}" target="_blank" class="px-4 py-2 text-gray-600 hover:text-pink-500 transition-colors">
            ...
            <div x-show="mobileMenuOpen" x-transition class="md:hidden border-t border-pink-100 bg-white">
                <div class="container mx-auto px-4 py-4 space-y-2">
                    <a href="{{ route('admin.dashboard') }}" @click="mobileMenuOpen = false" class="block px-4 py-3 rounded-xl transition-colors {{ request()->routeIs('admin.dashboard') ? 'bg-pink-100 text-pink-600 font-medium' : 'text-gray-700 hover:bg-pink-50' }}">
                        لوحة التحكم
                    </a>
                    <a href="{{ route('admin.products.index') }}" @click="mobileMenuOpen = false" class="block px-4 py-3 rounded-xl transition-colors {{ request()->routeIs('admin.products.*') ? 'bg-pink-100 text-pink-600 font-medium' : 'text-gray-700 hover:bg-pink-50' }}">
                        المنتجات
                    </a>
                    <a href="{{ route('admin.orders.index') }}" @click="mobileMenuOpen = false" class="block px-4 py-3 rounded-xl transition-colors {{ request()->routeIs('admin.orders.*') ? 'bg-pink-100 text-pink-600 font-medium' : 'text-gray-700 hover:bg-pink-50' }}">
                        الطلبات
                    </a>
                    <a href="{{ route('admin.reports.index') }}" @click="mobileMenuOpen = false" class="block px-4 py-3 rounded-xl transition-colors {{ request()->routeIs('admin.reports.*') ? 'bg-pink-100 text-pink-600 font-medium' : 'text-gray-700 hover:bg-pink-50' }}">
                        الحسابات
                    </a>
                    <a href="{{ route('home') }}" target="_blank" @click="mobileMenuOpen = false" class="block px-4 py-3 rounded-xl text-gray-700 hover:bg-pink-50 transition-colors">
                        عرض الموقع
                    </a>
