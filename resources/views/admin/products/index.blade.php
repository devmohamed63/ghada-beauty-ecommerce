...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <a href="{{ route('admin.products.create') }}" class="bg-gradient-to-r from-pink-500 to-teal-500 text-white px-6 py-3 rounded-full hover:shadow-lg transition-all flex items-center gap-2 justify-center">
    ...
    <div class="md:hidden space-y-4">
        @forelse($products as $product)
            @include('admin.components.product-card', ['product' => $product])
        @empty
            <div class="bg-white rounded-2xl shadow-sm border border-pink-100 p-12 text-center">
                ...
                <a href="{{ route('admin.products.create') }}" class="bg-gradient-to-r from-pink-500 to-teal-500 text-white px-6 py-3 rounded-full hover:shadow-lg transition-all inline-block">
                    إضافة منتج جديد
                </a>
            </div>
        @endforelse
                        ...
                        <tr class="hover:bg-pink-50 transition-colors">
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-3">
                                    <img src="{{ $product->getMainImageUrl('thumb') }}" alt="{{ $product->name }}" class="w-12 h-12 object-cover rounded-xl">
                                    <div>
                            ...
                            <td class="px-6 py-4">
                                <span class="font-bold text-pink-600">{{ number_format($product->price, 0) }}</span>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    <a href="{{ route('admin.products.show', $product) }}" class="p-2 text-teal-600 hover:bg-teal-50 rounded-lg transition-colors" aria-label="عرض المنتج" title="عرض">
                                    ...
                                    <a href="{{ route('admin.products.edit', $product) }}" class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" aria-label="تعديل المنتج" title="تعديل">
                                    ...
                                    <form action="{{ route('admin.products.destroy', $product) }}" method="POST" onsubmit="return confirm('هل أنت متأكد من حذف هذا المنتج؟')" class="inline">
                                        @csrf @method('DELETE')
